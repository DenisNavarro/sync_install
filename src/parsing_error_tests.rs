...
#[test]
fn pixi_download_url_in_a_previous_line() -> anyhow::Result<()> {
    parse_first_arg_and_check_error_contains(
        r#"RUN set -eux; \
            ...
            pixi --version"#,
        [
            "failed to parse line 6: ",
            "Pixi download URL already in a previous line: the Pixi version was 0.24.2",
        ],
    )
}

#[test]
fn pixi_download_url_without_expected_prefix() -> anyhow::Result<()> {
    parse_first_arg_and_check_error_contains(
        r#"RUN set -eux; \
            xh get --download \
                --follow "https://github.com/prefix-dev/pixi/releases/download/v0.25.0/pixi-$(uname -m)-unknown-linux-musl" \
                --output "$HOME/.pixi/bin/pixi"; \
            chmod +x "$HOME/.pixi/bin/pixi\"; \
            pixi --version"#,
        [
            "failed to parse line 3: ",
            "left trimmed line with Pixi download URL but which ",
            r#"does not start with "\"https://github.com/prefix-dev/pixi/releases/download/v""#,
        ],
    )
}

#[test]
fn pixi_download_url_without_expected_suffix() -> anyhow::Result<()> {
    parse_first_arg_and_check_error_contains(
        r#"RUN set -eux; \
            xh get --download --follow \
                "https://github.com/prefix-dev/pixi/releases/download/v0.25.0/pixi-x86_64-unknown-linux-musl" \
                --output "$HOME/.pixi/bin/pixi"; \
            chmod +x "$HOME/.pixi/bin/pixi"; \
            pixi --version"#,
        [
            "failed to parse line 3: ",
            "line with Pixi download URL but which ",
            r#"does not end with "/pixi-$(uname -m)-unknown-linux-musl\" \\""#,
        ],
    )
}

#[test]
fn pixi_download_url_with_empty_version() -> anyhow::Result<()> {
    parse_first_arg_and_check_error_contains(
        r#"RUN set -eux; \
            xh get --download --follow \
                "https://github.com/prefix-dev/pixi/releases/download/v/pixi-$(uname -m)-unknown-linux-musl" \
                --output "$HOME/.pixi/bin/pixi"; \
            chmod +x "$HOME/.pixi/bin/pixi"; \
            pixi --version"#,
        ["failed to parse line 3: ", "empty Pixi version"],
    )
}
